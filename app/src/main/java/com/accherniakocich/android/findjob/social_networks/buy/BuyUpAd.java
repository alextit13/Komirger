...
import com.accherniakocich.android.findjob.activities.MyAdds;
...
public class BuyUpAd extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        offer.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                AlertDialog.Builder builder = new AlertDialog.Builder(BuyUpAd.this);
            ...
            if (card != null) {
                ...
                cloudipsp.pay(card, order, new Cloudipsp.PayCallback() {
                    ...
                    public void onPaidProcessed(Receipt receipt) {
                        Toast.makeText(BuyUpAd.this, "Paid " + receipt.status.name() + "\nPaymentId:" + receipt.paymentId, Toast.LENGTH_LONG).show();
                    }
                    ...
                    public void onPaidFailure(Cloudipsp.Exception e) {
                        if (e instanceof Cloudipsp.Exception.Failure) {
                            ...
                            Toast.makeText(BuyUpAd.this, "Failure\nErrorCode: " +
                                    f.errorCode + "\nMessage: " + f.getMessage() + "\nRequestId: " + f.requestId, Toast.LENGTH_LONG).show();
                        } else if (e instanceof Cloudipsp.Exception.NetworkSecurity) {
                            Toast.makeText(BuyUpAd.this, "Network security error: " + e.getMessage(), Toast.LENGTH_LONG).show();
                        } else if (e instanceof Cloudipsp.Exception.ServerInternalError) {
                            Toast.makeText(BuyUpAd.this, "Internal server error: " + e.getMessage(), Toast.LENGTH_LONG).show();
                        } else if (e instanceof Cloudipsp.Exception.NetworkAccess) {
                            Toast.makeText(BuyUpAd.this, "Network error", Toast.LENGTH_LONG).show();
                        } else {
                            refreshUser();
                            Toast.makeText(BuyUpAd.this, "Платеж успешно произведен", Toast.LENGTH_LONG).show();
        ...
        FirebaseDatabase.getInstance().getReference().child("ads").child(ad.getDateAd()+"").removeValue().addOnCompleteListener(
                new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        long newData = Long.parseLong(ad.getDateAd()+"") - 86400000;
                        ad.setDateAd(newData);
                        FirebaseDatabase.getInstance().getReference().child("ads").child(ad.getDateAd()+"").setValue(ad);
                        Toast.makeText(BuyUpAd.this, "Объявление успешно поднято!", Toast.LENGTH_SHORT).show();
                    }
                }
        );
    }
